CREATE OR REPLACE
ALGORITHM = UNDEFINED VIEW `view_plastics_input_shares` (`event_id`,
`idx`,
`material_id`,
`input_quantity`,
`percentage`,
...
`output_epc_id_raw`) AS WITH `component_weights` AS (
SELECT
    `event_id` AS `event_id`,
    SUM(`weight_value`) AS `total_weight`
FROM
    `data`.`view_plastics_component_details`
GROUP BY
    `event_id`),
...
SELECT
    ...
    (`cd`.`weight_value` / `cw`.`total_weight` * 100) AS `percentage`,
    _UTF8MB4'P1' AS `unit`,
...
FROM
    `data`.`view_plastics_component_details` AS `cd`
...
FROM
    ...
    FROM
        `data`.`view_plastics_packaging_ingredients`
UNION ALL
    SELECT
        `event_id` AS `event_id`,
        `idx` AS `idx`,
        ...
        `epc_id_raw` AS `epc_id_raw`
    FROM
        `data`.`view_plastics_input_quantity_shares`
UNION ALL
    SELECT
        `event_id` AS `event_id`,
        `idx` AS `idx`,
...
-- `data`.view_plastics_inputs source