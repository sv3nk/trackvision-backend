WITH RECURSIVE cte_inputs AS (
    ...
    WHERE l1.parent_join_key = CONCAT('/01/', '04064074001970', '/10/', 'mBsLXFfTJe')
        AND l1.depth = 0
    UNION ALL
...
SELECT r.level,
    ...
    p.primary_image,
    p.production_step,
    s.description AS production_step_desc,
    p.quantity AS quantity_value,
    p.quantity_uom AS quantity_uom,
    IF (p.quantity_uom IS NULL, "units", quantity_uom.description) AS quantity_uom_desc,
    ...
    p.packaging_type,
    a.description AS packaging_type_desc,
    a.long_description AS packaging_type_long_desc,
    p.packaging_level,
    plc.description AS packaging_level_desc,
    plc.long_description AS packaging_level_long_desc,
    p.content_of_packaging,
    cpc.description AS content_of_packaging_desc,
    cpc.long_description AS content_of_packaging_long_desc,
    p.unit_length,
    p.unit_length_uom,
